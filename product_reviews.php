...
*/
  ...
  $product_info_query = tep_db_query("select p.products_id, p.products_model, i.image, p.products_price, p.products_tax_class_id, pd.products_name from " . TABLE_PRODUCTS . " p left join products_images i on (p.products_id = i.products_id and i.default_flag = '1'), " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = '" . (int)$HTTP_GET_VARS['products_id'] . "' and p.products_status = '1' and p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "'");
...
<?php
  if (tep_not_null($product_info['image'])) {
?>

  <div class="col-sm-4 text-center">
    <?php echo '<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $product_info['products_id']) . '">' . tep_image(DIR_WS_IMAGES . $product_info['image'], addslashes($product_info['products_name']), SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'hspace="5" vspace="5"') . '</a>'; ?>
