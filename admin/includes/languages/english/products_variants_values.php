<?php
/*
  $Id$

  osCommerce, Open Source E-Commerce Solutions
  http://www.oscommerce.com

  Copyright (c) 2015 osCommerce

  Released under the GNU General Public License
  */

define('HEADING_TITLE_VARIANT_VALUES', 'Products Variants Values');

define('TABLE_HEADING_ID', 'ID');
define('TABLE_HEADING_PRODUCT', 'Product Name');
define('TABLE_HEADING_PVG_TITLE', 'Title');
define('TABLE_HEADING_PVG_MODULE', 'Module');
define('TABLE_HEADING_VARIANTS_VALUE_NAME', 'Variant Value');
define('TABLE_HEADING_ACTION', 'Action');
define('TABLE_HEADING_SORT_ORDER', 'Sort Order');
define('IMAGE_VARIANTS_VALUES', 'Values');
define('MAX_ROW_LISTS_OPTIONS', 10);
define('TABLE_HEADING_VARIANTS_GROUP_NAME', 'Variants Group');

define('TEXT_WARNING_OF_DELETE', 'This option has products and values linked to it - it is not safe to delete it.');
define('TEXT_OK_TO_DELETE', 'This option has no products and values linked to it - it is safe to delete it.');
define('TEXT_OPTION_ID', 'Option ID');
define('TEXT_OPTION_NAME', 'Option Name');
?>