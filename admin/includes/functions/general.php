...
/// For variants Start
  function tep_get_vproducts_name($product_id, $language_id = 0) {
    global $languages_id;
  ...
  function tep_get_vproducts_description($product_id, $language_id) {
  ...
  function tep_get_vproducts_url($product_id, $language_id) {
  ...
  function tep_has_variants($product_id) {
    $product_query = tep_db_query("select has_children from products where products_id = '" . (int)$product_id . "'");
    $product = tep_db_fetch_array($product_query);
	
	$has_children = false;
    
	if ($product['has_children'] == 0) {
		$has_children = true;	
		tep_db_query("update products set has_children = '1' where products_id = '" . (int)$product_id . "'");
	}
    if ( ($product['has_children'] == 1) || ($has_children == true) ) {
		tep_db_query("update products set products_quantity = '0', products_price = '0.000', products_model = '', products_last_modified = now(), products_weight = '0.00', products_weight_class = '0', products_tax_class_id = '0' where products_id = '" . (int)$product_id . "'");
    }

  }
  
  function tep_has_still_variants($product_id) {
    $product_query = tep_db_query("select products_id from products where parent_id = '" . (int)$product_id . "'");
    
	if (!tep_db_num_rows($product_query)) {
      tep_db_query("update products set has_children = '0' where products_id = '" . (int)$product_id . "'");
    }
  }
  
	function tep_get_pvariants($product_id) {
		$pvariants_query = tep_db_query("select products_id from products where parent_id = '" . (int)$product_id . "'");		
		
		if (!tep_db_num_rows($pvariants_query)) {			
			
			$pvariants_array[] = array('variant_products_id' => (int)$product_id);    			
			}else{			
			$pvariants_array[] = array('variant_products_id' => (int)$product_id); 			
			
			while ($pvariants = tep_db_fetch_array($pvariants_query)) {				
				$pvariants_array[] = array('variant_products_id' => $pvariants['products_id']);
			}
		}
		return $pvariants_array;
	}
	
	function tep_truncate_products_variants_values($product_id) {
	$product_query = tep_db_query("select products_id from products where parent_id = '" . (int)$product_id . "'");

		if (tep_db_num_rows($product_query)) {
			while($truncate_values = tep_db_fetch_array($product_query)){
				
				tep_db_query('delete from products_variants where products_id = "' . (int)$truncate_values['products_id'] . '"');			
			}
		}
	}
	function tep_get_pvariants_options($product_id){
		global $languages_id;
		
		$output = NULL;
		
		$products = tep_db_query("select pv.products_variants_values_id, pvv.products_variants_groups_id, pvv.title, pvg.id as group_id, pvg.title as group_title from products_variants pv, products_variants_values pvv, products_variants_groups pvg where pv.products_id = '" . (int)$product_id . "' and pvv.languages_id = '" . (int)$languages_id . "'	and pvv.id = pv.products_variants_values_id and pvg.id = pvv.products_variants_groups_id and pvg.languages_id = '" . (int)$languages_id . "'");
		
		if (tep_db_num_rows($products)) {
			
			while($products_values = tep_db_fetch_array($products)){
					$output .= '<script>$("#variantValuesList_'.(int)$product_id.'").tagit({readOnly: true, listId: "'.$products_values['group_id'].'", fieldName: "products_variants_values_id['.(int)$product_id.']['. $products_values['products_variants_values_id'].']['. $products_values['group_id'].']"});</script>';
					$output .= '<script>$("#variantValuesList_'.(int)$product_id.'").tagit("createTag", "'.$products_values['group_title'].' : '. $products_values['title'].'");</script>';			
			}
		}
		return $output;
	...
	function tep_get_default_combo($product_id){
		
		$products_default_combo = tep_db_query("select default_combo from products_variants where products_id = '" . (int)$product_id . "' and default_combo = '1'");
		
		$output = NULL;
		
		if (tep_db_num_rows($products_default_combo)) {
			

			$output .= tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_checkbox_field('default_combo['.$product_id.']', 'on', 'true', '', 'default_combo') . '&nbsp;' . TEXT_DEFAULT_COMBO; 

		}else{
			
			$output .= tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_checkbox_field('default_combo['.$product_id.']', '', '', '', 'default_combo') . '&nbsp;' . TEXT_DEFAULT_COMBO;
