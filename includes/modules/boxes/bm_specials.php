	...
	*/
	...
	class bm_specials {
		...
		function execute() {
			...
			if ($random_product = tep_random_select("select p.products_id, p.products_price, p.products_tax_class_id, pd.products_name, pd.products_keyword, s.specials_new_products_price, i.image from products p left join products_images i on (p.products_id = i.products_id and i.default_flag = '1'), products_description pd, specials s where s.status = '1' and s.products_id = p.products_id and p.products_status = '1' and p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "' order by s.specials_date_added desc limit " . MAX_RANDOM_SELECT_SPECIALS)){
				ob_start();
