...
*/
  ...
  class bm_categories {
    ...
    function execute() {
      global $oscTemplate,$osC_CategoryTree, $cPath;

      $osC_CategoryTree->setCategoryPath($cPath, '<strong>', '</strong>');
      $osC_CategoryTree->setSpacerString('&nbsp;&nbsp;', 1);

      $osC_CategoryTree->setParentGroupString('<ul class="nav nav-pills nav-stacked">', '</ul>', true);
      
      $category_tree = $osC_CategoryTree->getTree();
