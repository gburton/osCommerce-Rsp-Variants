<?php
	...
	*/
	...
	class osC_Products {
		...
		function execute() {
			global $osC_Database, $languages_id, $osC_CategoryTree, $osC_Image;
			...
			//$listing_sql = "select p.products_id from products p left join product_attributes pa on (p.products_id = pa.products_id) left join templates_boxes tb on (pa.id = tb.id and tb.code = 'manufacturers'), products_description pd, categories c, products_to_categories p2c where p.products_status = 1 and p.products_id = pd.products_id and pd.language_id = " . (int)$languages_id . " and p.products_id = p2c.products_id and p2c.categories_id = c.categories_id " . $listing_sql_Ext . " ";
			$listing_sql = "select distinct p.products_id, p.products_price, p.products_tax_class_id, IF(s.status, s.specials_new_products_price, NULL) as specials_new_products_price, IF(s.status, s.specials_new_products_price, p.products_price) as final_price from products p left join product_attributes pa on (p.products_id = pa.products_id) left join templates_boxes tb on (pa.id = tb.id and tb.code = 'manufacturers') left join manufacturers m on p.manufacturers_id = m.manufacturers_id left join specials s on p.products_id = s.products_id, products_description pd, categories c, products_to_categories p2c where p.parent_id = 0 and p.products_status = 1 and p.products_id = pd.products_id and pd.language_id = " . (int)$languages_id . " and p.products_id = p2c.products_id and p2c.categories_id = c.categories_id " . $listing_sql_Ext . " ";
