	...
	class order {
		...
		function cart() {
			...
			foreach ($cart->get_products() as $products) {
				$this->products[$index] = array('qty' => $products['quantity'],
				'name' => $products['name'],
				'model' => $products['model'],
				'tax' => tep_get_tax_rate($products['tax_class_id'], $tax_address['entry_country_id'], $tax_address['entry_zone_id']),
				'tax_description' => tep_get_tax_description($products['tax_class_id'], $tax_address['entry_country_id'], $tax_address['entry_zone_id']),
				'price' => $products['price'],
				'final_price' => $products['price'],
				'weight' => $products['weight'],
				'id' => $products['id']);
			
				if ( $cart->isVariant($products['item_id']) ) {
					$subindex = 0;
					foreach ( $cart->getVariant($products['item_id']) as $attributes ) {
					
						//reset($products['attributes']);

						$this->products[$index]['attributes'][$subindex] = array('option' => $attributes['group_title'],
						'value' => $attributes['value_title']);
